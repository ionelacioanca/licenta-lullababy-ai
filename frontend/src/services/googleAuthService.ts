...
export const checkGoogleUser = async (accessToken: string) => {
  try {
    ...
    const response = await fetch('http://192.168.1.50:5000/api/users/auth/google/check', {
      method: 'POST',
      headers: {
...
export const registerWithGoogle = async (accessToken: string, googleData: any, role: string, customRole?: string) => {
  try {
    const response = await fetch('http://192.168.1.50:5000/api/users/auth/google/register', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
