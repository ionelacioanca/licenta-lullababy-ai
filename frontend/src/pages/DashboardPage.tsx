...
const DashboardPage: React.FC = () => {
  ...
  const loadNotificationsCount = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://192.168.1.50:5000/api/alerts/baby/${babyId}/unread-count`,
        {
          headers: {
  ...
  const loadBabyForParent = async () => {
    ...
    try {
      const userInfoResponse = await fetch(`http://192.168.1.50:5000/api/user-info`, {
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      const checkNotifications = async () => {
        try {
          await fetch('http://192.168.1.50:5000/api/calendar/trigger-notifications', {
            method: 'POST',
          });
                  ...
                  <View key={memory._id}>
                    ...
                    <View style={styles.memoryRow}>
                      ...
                      {memory.photos.length > 0 && (
                        <Image
                          source={{ uri: `http://192.168.1.50:5000${memory.photos[0]}` }}
