...
const ChildProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    if (baby) {
      ...
      const newAvatarImage = baby.avatarImage 
        ? (baby.avatarImage.startsWith('http') ? baby.avatarImage : `http://192.168.1.11:5000${baby.avatarImage}`)
        : null;
  ...
  const loadBabyProfile = async () => {
    try {
      ...
      const response = await fetch(
        `http://192.168.1.11:5000/api/baby/parent/${parentId}`,
        {
          headers: {
      ...
      if (data && Array.isArray(data)) {
        ...
        if (selectedBaby) {
          ...
          setAvatarImage(selectedBaby.avatarImage ? `http://192.168.1.11:5000${selectedBaby.avatarImage}` : null);
        } else {
  ...
  const uploadAvatarImage = async (imageUri: string) => {
    try {
      ...
      const response = await fetch(
        `http://192.168.1.11:5000/api/baby/${selectedBabyId}/avatar`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      const fullImageUrl = updatedBaby.avatarImage ? `http://192.168.1.11:5000${updatedBaby.avatarImage}` : null;
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      const response = await fetch(
        `http://192.168.1.11:5000/api/baby/${selectedBabyId}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
