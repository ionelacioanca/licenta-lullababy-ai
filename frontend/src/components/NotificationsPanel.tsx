...
export default function NotificationsPanel({
  ...
  const loadAlerts = async () => {
    try {
      ...
      const response = await fetch(
        `http://192.168.1.20:5000/api/alerts/baby/${babyId}`,
        {
          headers: {
  ...
  const markAsRead = async (alertId: string) => {
    try {
      ...
      const response = await fetch(
        `http://192.168.1.20:5000/api/alerts/${alertId}/read`,
        {
          method: 'PUT',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const markAllAsRead = async () => {
    try {
      ...
      const response = await fetch(
        `http://192.168.1.20:5000/api/alerts/baby/${babyId}/read-all`,
        {
          method: 'PUT',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const deleteAlert = async (alertId: string) => {
    try {
      ...
      const response = await fetch(
        `http://192.168.1.20:5000/api/alerts/${alertId}`,
        {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${token}`,
